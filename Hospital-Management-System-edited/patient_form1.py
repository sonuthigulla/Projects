...
class Patient:
    def __init__(self, master):
        ...
        self.lblsex = Label(self.LoginFrame, text="Gender", font="Helvetica 14 bold", bg="cadet blue", bd=22)
        self.lblsex.grid(row=2, column=0)
        # Create a list of gender options for the dropdown
        gender_options = ["Male", "Female", "Other"]  # Add your actual gender options here

        # Create a StringVar to store the selected value
        self.selected_gender = tk.StringVar()

        # Create the Combobox and set its values
        self.gender_combobox = ttk.Combobox(self.LoginFrame, textvariable=self.selected_gender, values=gender_options,
        ...
        self.lblDOB = Label(self.LoginFrame, text="DOB (MM-DD-YYYY)", font="Helvetica 14 bold", bg="cadet blue", bd=22)
        self.lblDOB.grid(row=3, column=0)
        self.dob_entry = DateEntry(self.LoginFrame, font="Helvetica 14 bold", date_pattern="mm-dd-yyyy")
        ...
        # Create a list of blood groups for the dropdown
        blood_group_options = ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"]  # Add your actual blood groups here

        # Create a StringVar to store the selected value
        self.selected_blood_group = tk.StringVar()

        # Create the Combobox and set its values
        self.blood_group_combobox = ttk.Combobox(self.LoginFrame, textvariable=self.selected_blood_group,
                                                 values=blood_group_options, font="Helvetica 14 bold", state="readonly")
        self.blood_group_combobox.grid(row=4, column=1)
        ...
        self.lbldoc = Label(self.LoginFrame, text="CONSULTING DOCTOR", font="Helvetica 14 bold", bg="cadet blue", bd=22)
        self.lbldoc.grid(row=3, column=2)
        # Create a list of options for the dropdown
        doctor_options = ["Shashi", "Govind", "Manoj", "Sonu"]  # Add your actual doctor names here

        # Create a StringVar to store the selected value
        self.selected_doctor = tk.StringVar()

        # Create the Combobox and set its values
        self.doctor_combobox = ttk.Combobox(self.LoginFrame, textvariable=self.selected_doctor, values=doctor_options,
                                            font="Helvetica 14 bold", state="readonly")
        self.doctor_combobox.grid(row=3, column=3)
        ...
        # ===========BUTTONS=============
        self.button2 = Button(self.LoginFrame2, text="Create", width=10, font="Helvetica 14 bold", bg="cadet blue",
        ...
        self.button6 = Button(self.LoginFrame2, text="BACK", width=10, font="Helvetica 14 bold", bg="cadet blue",
                              command=self.Exit)
    ...
    def INSERT_PAT(self):
        ...
        selected_dob = self.dob_entry.get()
        selected_blood_group = self.selected_blood_group.get()
        ...
        if x != 0:
            ...
            cursor.execute(
                'INSERT INTO PATIENT (NAME, SEX, BLOOD_GROUP, DOB, ADDRESS, CONSULT_TEAM, EMAIL) VALUES (?, ?, ?, ?, ?, ?, ?)',
                (p2, p3, p4, p5, p8, p9, p10))

            # Get the last inserted PATIENT_ID immediately after the INSERT statement
            last_patient_id = cursor.lastrowid

            # Check if the PATIENT_ID already exists in CONTACT_NO
            existing_contact = list(conn.execute("SELECT * FROM CONTACT_NO WHERE PATIENT_ID =?", (last_patient_id,)))

            if not existing_contact:
                cursor.execute('INSERT INTO CONTACT_NO VALUES (?,?,?)', (last_patient_id, p6, p7,))
                tkinter.messagebox.showinfo("HOSPITAL DATABASE SYSTEM", "DETAILS INSERTED INTO DATABASE")
            else:
                tkinter.messagebox.showerror("HOSPITAL DATABASE SYSTEM", "PATIENT_ID already exists in CONTACT_NO")
    ...
    def SEARCH_PAT(self):
        ...
        if (len(p) == 0):
            ...
            for i in t:
                ...
                self.l4 = Label(self.LoginFrame, text="DOB (MM-DD-YYYY)", font="Helvetica 14 bold", bg="cadet blue",
                                bd=22)
