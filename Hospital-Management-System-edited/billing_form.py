...
class Billing:
    def __init__(self,master):
        ...
        self.lbldid = Label(self.LoginFrame,text="DATE DISCHARGED(MM-DD-YYYY)",font="Helvetica 14 bold",bg="cadet blue",bd=22)
        self.lbldid.grid(row=1,column=0)
        self.date_discharged_entry = DateEntry(self.LoginFrame, font="Helvetica 14 bold", date_pattern="mm-dd-yyyy")
        self.date_discharged_entry.grid(row=1, column=1)
    ...
    def UPDATE_DATE(self):
        global b1,b2
        selected_date_discharged = self.date_discharged_entry.get()
        conn = sqlite3.connect("HospitalDB.db")
        conn.cursor()
        b1 = (self.P_id.get())
        # b2 =(self.dd.get())
        conn.execute("UPDATE ROOM SET DATE_DISCHARGED=? where PATIENT_ID=?", (selected_date_discharged, b1,))
        conn.commit()
