...
class Appointment:
    def __init__(self,master):
        ...
        self.lblapt = Label(self.LoginFrame,text="APPOINTMENT TIME",font="Helvetica 14 bold",bg="cadet blue",bd=22)
        self.lblapt.grid(row=0,column=2)
        self.ap_time_var = tk.StringVar()
        appointment_times = ['09:00:00', '10:00:00', '11:00:00', '12:00:00', '13:00:00', '14:00:00']  # Example times
        self.lblapt_combobox = ttk.Combobox(self.LoginFrame, font="Helvetica 14 bold", textvariable=self.ap_time_var,
                                            values=appointment_times, state="readonly")
        self.lblapt_combobox.grid(row=0, column=3)
        # self.lblapt  = Entry(self.LoginFrame,font="Helvetica 14 bold",bd=2,textvariable=self.ap_time )
        # self.lblapt.grid(row=0,column=3)

        self.lblapd = Label(self.LoginFrame,text="APPOINTMENT DATE",font="Helvetica 14 bold",bg="cadet blue",bd=22)
        self.lblapd.grid(row=1,column=2)
        self.pat_dob = tk.StringVar()
        self.lblDOB_entry = DateEntry(self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.pat_dob,
                                      date_pattern='mm-dd-yyyy')
        self.lblDOB_entry.grid(row=1, column=3)
        ...
        self.button6 = Button(self.LoginFrame2, text="BACK",width =10,font="Helvetica 14 bold",bg="cadet blue",command = self.Exit)
    ...
    def INSERT_AP(self):
        ...
        # e5=(self.ap_date.get())
        e6=(self.des.get())
        selected_appointment_time = self.ap_time_var.get()
        selected_appointment_date = self.pat_dob.get()
        conn = sqlite3.connect("HospitalDB.db")
        ...
        if x!=0:
            ...
            conn.execute("Insert into appointment values(?,?,?,?,?,?)",(e1,e2,e3,selected_appointment_time,selected_appointment_date,e6,))
...
class SEA_AP:
    def __init__(self,master):    
        ...
        #===========LABELS=============          
        self.lblpatid = Label(self.LoginFrame,text="ENTER DATE TO VIEW APPOINTMENTS(MM-DD-YYYY)",font="Helvetica 14 bold",bg="cadet blue",bd=22)
        self.lblpatid.grid(row=0,column=0)
