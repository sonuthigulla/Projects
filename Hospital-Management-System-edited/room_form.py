...
class Room:
    ...
    def __init__(self,master):
        ...
        # Create a list of room type options for the dropdown
        room_type_options = ["Single Room", "Twin Sharing", "Triple Sharing"]  # Add your actual room type options here

        # Create a StringVar to store the selected value
        self.selected_room_type = tk.StringVar()

        # Create the Combobox and set its values
        self.room_type_combobox = ttk.Combobox(self.LoginFrame, textvariable=self.selected_room_type,
                                               values=room_type_options, font="Helvetica 14 bold", state="readonly")
        self.room_type_combobox.grid(row=1, column=1)
        ...
        self.date_admitted_entry = DateEntry(self.LoginFrame, font="Helvetica 14 bold", date_pattern="mm-dd-yyyy")
        self.date_admitted_entry.grid(row=1, column=3)
    ...
    def INSERT_ROOM(self):
        ...
        selected_date_admitted = self.date_admitted_entry.get()
        conn = sqlite3.connect("HospitalDB.db")
        conn.cursor()
        r1=(self.P_id.get())
        # r2=(self.room_t.get())
        ...
        if x!=0:
            ...
            conn.execute('INSERT INTO ROOM VALUES(?,?,?,?,?,?)',(r1, r3,selected_room_type, r4, selected_date_admitted,r6,))
